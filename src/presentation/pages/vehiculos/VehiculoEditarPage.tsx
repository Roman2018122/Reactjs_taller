...
export default function VehiculoEditarPage() {
  ...
  const getVehiculoById = useVehiculoStore(
    (state) => state.getById,
  );

  const updateVehiculo = useVehiculoStore(
    (state) => state.update,
  );
  ...
  useEffect(() => {
    ...
    void getVehiculoById(vehiculoId);
  ...
  useEffect(() => {
    if (!vehiculo || vehiculo.id !== vehiculoId) {
      return;
    }
    ...
    <main className="mx-auto w-full max-w-7xl space-y-6 px-4 py-6 sm:px-6 lg:px-8">
      <header className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <p className="mt-2 text-sm text-slate-600">
          Modifica la información registrada del
          vehículo.
        </p>
      </header>
      ...
      <Card className="max-w-3xl overflow-hidden border-slate-200 bg-white shadow-sm">
        <CardHeader className="border-b border-slate-200 bg-slate-50">
          ...
          <CardDescription className="text-slate-600">
            Actualiza los campos necesarios y guarda
            los cambios.
          </CardDescription>
        </CardHeader>
